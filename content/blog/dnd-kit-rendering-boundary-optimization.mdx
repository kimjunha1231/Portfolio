...
```text
DndContext (드래그 센서·충돌 검출·글로벌 드래그 상태 관리)
  │
  └── SortableContext (items 목록·strategy·정렬 좌표 Context 전파)
        │
        ├── SortableCard A (useSortable 구독) ── 카드 UI (회사명·직무·마감일)
        ├── SortableCard B (useSortable 구독) ── 카드 UI (회사명·직무·마감일)
        └── SortableCard C (useSortable 구독) ── 카드 UI (회사명·직무·마감일)
```
...
```tsx
// useSortable.ts의 중요한 부분을 개념적으로 줄인 코드입니다.
const {
  items,
  activeIndex,
  overIndex,
...
```text
[1] Pointer Sensor (pointermove 이벤트 발생)
      ↓
[2] DndContext (active · over · 드래그 측정값 계산)
      ↓
[3] SortableContext (새로운 Context value 생성 후 하위 전파)
      ↓
[4] useSortable 카드들 (Context 구독으로 인해 transform · transition 재계산)
      ↓
[5] 카드 UI 콘텐츠 (동일 컴포넌트에 위치한 무거운 카드 UI도 함께 재실행)
```
...
function SortableCard({ application }: { application: Application }) {
  ...
  // ⚠️ transform이 바뀌지 않은 카드라도 Context 변경으로 인해 이 컴포넌트 전체가 다시 실행됩니다.
  return (
    <article
      ref={setNodeRef}
...
```tsx
// ❌ 문제 구조: React.memo로 감싸도 내부에서 Context를 직접 구독하면 재렌더링을 막을 수 없습니다.
const SortableCard = memo(function SortableCard({ application }: Props) {
  // useSortable Hook이 상위 SortableContext의 변경을 직접 읽음
  const sortable = useSortable({ id: application.id });

  // application Props가 동일해도 Context가 변하면 이 컴포넌트는 다시 실행됩니다.
...
```tsx
// ✅ 개선 구조: 드래그 상태를 아는 외곽 셸과 순수 UI 콘텐츠를 분리

// 1. 순수 카드 UI 콘텐츠 (Context에 의존하지 않는 메모이제이션된 컴포넌트)
...
// 2. 외곽 드래그 셸 (useSortable Context 구독 및 포지션 스타일 계산 전담)
function SortableCardShell({ application, onDelete }: Props) {
  ...
  return (
    <article
      ...
      {...listeners}
    >
      {/* 셸이 재실행되어도 Props 참조가 같으면 CardContent 렌더링은 건너끕니다 */}
      <CardContent
