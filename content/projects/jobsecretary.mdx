...
문제는 드래그 위치를 계산하는 로직과 회사명·직무·마감일을 그리는 UI가 같은 컴포넌트에 있었던 점입니다. 그래서 드래그 중 바뀌지 않는 카드 콘텐츠도 같은 렌더링 범위에 포함되었습니다. `dnd-kit`의 Context 구조와 이 문제를 해결한 과정은 [**dnd-kit에서 렌더링 경계를 분리하는 방법**](/blog/dnd-kit-rendering-boundary-optimization)에 정리했습니다.
