const g={1:{title:"Dearfam : 가족의 이야기를 담아드립니다",period:"2024.11 ~ 현재",role:`
      ...
      <ul>
		...
		`,github:"https://github.com/gdsc-ssu/poppet-app"}};function f(){const e={threshold:.1,rootMargin:"0px 0px -50px 0px"},s=new IntersectionObserver(i=>{i.forEach(a=>{a.isIntersecting&&a.target.classList.add("visible")})},e);document.querySelectorAll(".fade-in, .slide-in-left, .slide-in-right").forEach(i=>s.observe(i))}function b(){document.querySelectorAll('a[href^="#"]').forEach(e=>{e.addEventListener("click",s=>{s.preventDefault();const r=e.getAttribute("href");if(r){const i=document.querySelector(r);i&&i.scrollIntoView({behavior:"smooth",block:"start"})}})})}function y(){console.log("모달 기능 초기화 시작");const e=document.getElementById("project-modal"),s=document.getElementById("modal-title"),r=document.getElementById("modal-role"),i=document.getElementById("modal-achievements"),a=document.getElementById("modal-details"),n=document.getElementById("modal-github"),d=document.getElementById("modal-close"),c=document.getElementById("modal-period-text");console.log("모달 요소들:",{modal:!!e,modalTitle:!!s,modalPeriod:!!c,modalRole:!!r,modalAchievements:!!i,modalDetails:!!a,modalGithub:!!n,modalClose:!!d});function p(l){console.log("모달 열기 시도:",l);const t=g[l];if(!t){console.error("프로젝트 데이터를 찾을 수 없습니다:",l);return}console.log("프로젝트 데이터:",t),s&&c&&r&&i&&a&&n&&e?(s.textContent=t.title,c.textContent=`개발 기간: ${t.period}`,t.github?(n.href=t.github,n.style.display="inline-flex"):n.style.display="none",console.log("모달 링크 설정:",{github:t.github,githubElement:n.href}),r&&(r.innerHTML=t.role),i&&(i.innerHTML=t.achievements),a&&(a.innerHTML=t.details),document.body.style.overflow="hidden",e.classList.add("show"),setTimeout(()=>{const o=e.querySelector(".modal-content");o&&(o.scrollTop=0,o.setAttribute("tabindex","-1"),o.focus(),o.scrollIntoView({behavior:"instant",block:"start"}))},100),console.log("모달 열림 완료")):console.error("모달 요소가 없습니다")}function m(){console.log("모달 닫기"),e&&(e.classList.remove("show"),document.body.style.overflow="")}const u=document.querySelectorAll(".project-card");console.log("프로젝트 카드 개수:",u.length),u.forEach((l,t)=>{const o=l.getAttribute("data-project-id");console.log(`카드 ${t+1}의 project-id:`,o),l.addEventListener("click",function(h){h.preventDefault(),console.log("카드 클릭됨:",o),o?p(o):console.error("프로젝트 ID가 없습니다:",l)})}),d&&d.addEventListener("click",m),e&&e.addEventListener("click",function(l){l.target===e&&!l.target.closest(".btn")&&!l.target.closest(".modal-links")&&m()}),n&&n.addEventListener("click",function(l){l.preventDefault(),l.stopPropagation(),console.log("GitHub 링크 클릭됨:",this.href),this.href&&this.href!=="#"&&this.href!=="javascript:void(0)"&&this.href!==window.location.href+"#"&&this.href.startsWith("http")&&window.open(this.href,"_blank","noopener,noreferrer")}),document.addEventListener("keydown",function(l){l.key==="Escape"&&e&&e.classList.contains("show")&&m()}),console.log("모달 기능 초기화 완료")}document.addEventListener("DOMContentLoaded",function(){f(),b(),y()});
